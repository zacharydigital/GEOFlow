...
@section('content')
    <div class="mx-auto max-w-5xl px-4 sm:px-0">
        <header class="mb-6 flex items-start gap-4 sm:mb-8">
            ...
            <div class="min-w-0">
                <h1 class="text-balance text-2xl font-bold text-gray-900">{{ __('admin.ai_source_providers.modal_edit') }}</h1>
                <p class="mt-1 max-w-3xl text-pretty text-sm leading-6 text-gray-600">{{ __('admin.ai_source_providers.search_list_desc') }}</p>
            </div>
        </header>

        <form method="POST" action="{{ route('admin.ai-source-providers.update', ['providerId' => $provider['id']]) }}" class="overflow-hidden rounded-xl bg-white shadow">
            @csrf
            @method('PUT')
            ...
            <div class="border-b border-gray-200 px-5 py-5 sm:px-6">
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0">
                        <h2 class="break-words text-lg font-semibold text-gray-900">{{ $provider['name'] }}</h2>
                        <p class="mt-1 text-pretty text-sm leading-6 text-gray-600">{{ __('admin.ai_source_providers.api_key_help_edit') }}</p>
                    </div>
                </div>
            </div>

            <div class="px-5 py-6 sm:px-6">
                <x-admin.ai-source-provider-form-fields mode="edit" :provider="$provider" :default-endpoint="$defaultDoubaoEndpoint" />
            </div>
