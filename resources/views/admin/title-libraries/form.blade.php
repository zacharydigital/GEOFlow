...
@php
    ...
    $returnUrl = $isEdit && $contextValue === 'detail'
        ? route('admin.title-libraries.detail', ['libraryId' => (int) $libraryId])
...
@section('content')
    <div class="mx-auto max-w-4xl px-4 sm:px-0">
        <header class="mb-6 flex items-start gap-4 sm:mb-8">
            ...
            <div class="min-w-0">
                <h1 class="text-balance break-words text-2xl font-bold text-gray-900">{{ $isEdit ? __('admin.button.edit').' · '.$nameFallback : __('admin.title_libraries.modal_create') }}</h1>
                <p class="mt-1 max-w-3xl text-pretty text-sm leading-6 text-gray-600">{{ __('admin.title_libraries.subtitle') }}</p>
            </div>
        </header>
        ...
        <form method="POST" action="{{ $formAction }}" class="overflow-hidden rounded-xl bg-white shadow" data-library-entry-form data-processing-label="{{ __('admin.message.processing') }}">
            ...
            <div class="border-b border-gray-200 px-5 py-5 sm:px-6">
                ...
                <p class="mt-1 text-sm leading-6 text-gray-600">{{ __('admin.title_libraries.subtitle') }}</p>
            </div>

            <div class="space-y-6 px-5 py-6 sm:px-6">
                <div>
                    <label for="name" class="block text-sm font-semibold text-gray-800">{{ __('admin.title_libraries.field_name') }}</label>
                    <input id="name" type="text" name="name" required autofocus maxlength="100" autocomplete="off" value="{{ $nameValue }}" @error('name') aria-invalid="true" aria-describedby="title-library-name-error" @enderror class="mt-2 block min-h-11 w-full rounded-lg border-gray-300 px-3 py-2.5 text-sm text-gray-900 shadow-sm focus:border-green-500 focus:ring-green-500" placeholder="{{ __('admin.title_libraries.placeholder_name') }}">
                    @error('name')
                        <p id="title-library-name-error" class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                <div>
                    <label for="description" class="block text-sm font-semibold text-gray-800">{{ __('admin.title_libraries.field_description') }}</label>
                    <textarea id="description" name="description" rows="5" @error('description') aria-invalid="true" aria-describedby="title-library-description-error" @enderror class="mt-2 block w-full resize-y rounded-lg border-gray-300 px-3 py-3 text-sm leading-6 text-gray-900 shadow-sm focus:border-green-500 focus:ring-green-500" placeholder="{{ __('admin.title_libraries.placeholder_description') }}">{{ $descriptionValue }}</textarea>
                    @error('description')
                        <p id="title-library-description-error" class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                @error('context')
